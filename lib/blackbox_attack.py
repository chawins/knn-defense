...
# from models import (CIFAR10, IMAGENET, MNIST, load_cifar10_data,
#                     load_imagenet_data, load_mnist_data, load_model,
#                     show_image)
...
def attack_targeted(model, train_dataset, x0, y0, target, alpha=0.1, beta=0.001, iterations=1000):
    ...
    for i, (xi, yi) in enumerate(train_dataset):
        ...
        if model.predict(xi) == target:
            ...
            lbd, count = fine_grained_binary_search_targeted(
                model, x0, y0, target, theta, initial_lbd)
            query_count += count
            if lbd < g_theta:
    ...
    for i in range(iterations):
        ...
        for _ in range(q):
            ...
            g1, count = fine_grained_binary_search_local_targeted(
                model, x0, y0, target, ttt, initial_lbd=g2, tol=beta / 500)
            opt_count += count
        ...
        if (i + 1) % 50 == 0:
            print("Iteration %3d: g(theta + beta*u) = %.4f g(theta) = %.4f distortion %.4f num_queries %d" %
                  (i + 1, g1, g2, torch.norm(g2 * theta), opt_count))
...
def attack_mnist(alpha=0.2, beta=0.001, isTarget=False, num_attacks=100):
    ...
    def single_attack(image, label, target=None):
        ...
        if target == None:
            adversarial = attack_untargeted(
                model, dataset, image, label, alpha=alpha, beta=beta, iterations=1000)
        else:
            print("Targeted attack: %d" % target)
            adversarial = attack_targeted(
                model, dataset, image, label, target, alpha=alpha, beta=beta, iterations=1000)
    ...
    print("\n\n Running {} attack on {} random  MNIST test images for alpha= {} beta= {}\n\n".format(
        "targetted" if isTarget else "untargetted", num_attacks, alpha, beta))
    total_distortion = 0.0

    samples = [6312, 6891, 4243, 8377, 7962, 6635, 4970, 7809, 5867, 9559, 3579, 8269, 2282, 4618, 2290, 1554, 4105, 9862, 2408, 5082, 1619, 1209, 5410, 7736, 9172, 1650, 5181, 3351, 9053, 7816, 7254, 8542, 4268, 1021, 8990, 231, 1529, 6535, 19, 8087, 5459, 3997, 5329, 1032, 3131,
               9299, 3910, 2335, 8897, 7340, 1495, 5244, 8323, 8017, 1787, 4939, 9032, 4770, 2045, 8970, 5452, 8853, 3330, 9883, 8966, 9628, 4713, 7291, 9770, 6307, 5195, 9432, 3967, 4757, 3013, 3103, 3060, 541, 4261, 7808, 1132, 1472, 2134, 634, 1315, 8858, 6411, 8595, 4516, 8550, 3859, 3526]
...
def attack_cifar10(alpha=0.2, beta=0.001, isTarget=False, num_attacks=100):
    ...
    samples = [6311, 6890, 663, 4242, 8376, 7961, 6634, 4969, 7808, 5866, 9558, 3578, 8268, 2281, 2289, 1553, 4104, 8725, 9861, 2407, 5081, 1618, 1208, 5409, 7735, 9171, 1649, 5796, 7113, 5180, 3350, 9052, 7253, 8541, 4267, 1020, 8989, 230, 1528, 6534, 18, 8086, 3996, 1031, 3130, 9298, 3632, 3909,
               2334, 8896, 7339, 1494, 5243, 8322, 8016, 1786, 9031, 4769, 8969, 5451, 8852, 3329, 9882, 8965, 9627, 4712, 7290, 9769, 6306, 5194, 3966, 4756, 3012, 3102, 540, 4260, 7807, 1471, 2133, 2450, 633, 1314, 8857, 6410, 8594, 4515, 8549, 3858, 3525, 6411, 4360, 7753, 7413, 684, 3343, 6785, 7079, 2263]
...
def attack_imagenet(arch='resnet50', alpha=0.2, beta=0.001, isTarget=False, num_attacks=100):
    ...
    def attack_single(image, label, target=None):
        ...
        if target == None:
            adversarial = attack_untargeted(
                model, dataset, image, label, alpha=alpha, beta=beta, iterations=1500)
        else:
            print("Targeted attack: %d" % target)
            adversarial = attack_targeted(
                model, dataset, image, label, target, alpha=alpha, beta=beta, iterations=1500)
        print("Predicted label for adversarial example: ",
              model.predict(adversarial))
        return torch.norm(adversarial - image)

    print("\nRunning {} attack on {} random IMAGENET test images for alpha= {} beta= {} using {}\n".format(
        "targetted" if isTarget else "untargetted", num_attacks, alpha, beta, arch))
    total_distortion = 0.0

    samples = [25248, 27563, 2654, 16969, 31846, 26538, 19878, 14316, 33076, 9128, 9159, 49533, 34903, 46215, 963220326, 6473, 483344826, 216406600, 23187,
               40036, 41971, 13401, 36211, 31262, 4082, 35960, 6113, 47167, 46548, 75, 40102, 32348, 21313, 46114, 4128, 37193, 14530, 9339, 5978, 20976, 33289]
    for idx in samples:
        #idx = random.randint(100, len(test_dataset)-1)
        image, label = test_dataset[idx]
        print("\n\n======== Image %d =========" % idx)
        target = None if not isTarget else random.choice(
            list(range(label)) + list(range(label + 1, 1000)))
